...
const Song = ({ id, name, artist, album, duration, img, index }) => {
  ...
  return (
    <div
      className={`${
        isSongPlaying && id === currentSong.id ? "bg-[#19177F]" : ""
      } group flex flex-row items-center justify-between gap-2 text-left mx-auto mb-[25px] px-[9px] py-[7px] hover:bg-[#19177F] w-full`}
    >
      <p>{index + 1}</p>

      <div className="block relative cursor-pointer song-img-container h-[47.33px] my-[0px] mx-[0px] w-[56.52px] overflow-hidden">
        {isSongPlaying && id === currentSong.id ? (
          <FaPause
            onClick={() => {
              handleSongPlay(id);
            }}
            className="absolute text-white text-[20px] top-[30%] left-[30%] z-10"
          />
        ) : (
          <FaPlay
            onClick={() => {
        ...
        <img
          className={`${
            isSongPlaying && id === currentSong.id ? "opacity-40" : ""
          } object-cover z-7 group-hover:opacity-60`}
          src={img}
          alt=""
        />
      </div>
